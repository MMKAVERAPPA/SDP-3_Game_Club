...
export default function ManageGames() {
    ...
    const addGame = async () => {
        try {
            await API.post("/games", { name, price, description: desc, status, duration, min_player_count: minPlayers, max_player_count: maxPlayers, multiple_player_count: multi });
            setName(""); setPrice(""); setDesc(""); setStatus("active"); setDuration(""); setMinPlayers(""); setMaxPlayers(""); setMulti("");
            fetchGames();
        } catch (err) {
            console.error(err);
        }
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
                ...
                <input type="number" placeholder="Multi Player Count" className="border p-2 rounded" value={multi} onChange={(e) => setMulti(e.target.value)} />
            ...
            <table className="w-full border">
                <thead>
                    <tr className="bg-gray-800 text-white">
                        ...
                        <th className="p-2 border">Multi Count</th>
                ...
                <tbody>
                    {games.map((g) => (
                        <tr key={g._id}>
                            ...
                            <td className="border p-2">{g.min_player_count}</td>
                            <td className="border p-2">{g.max_player_count}</td>
                            <td className="border p-2">{g.multiple_player_count}</td>
                            <td className="border p-2">
                                <button className="bg-red-500 text-white px-2 py-1 rounded" onClick={() => deleteGame(g._id)}>Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
